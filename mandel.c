...
void image_save_rgb(struct render *set);
...
int load_config(struct render *set, char *config_file){
	...
	fscanf(fin, "%s", buff);
	strcpy(set->basename, buff);


	fscanf(fin, "%s", buff);
	ptr = strtok(buff, size_delim);
	if(ptr ==NULL){return 0;}
	set->width = strtod(ptr, &p);

	ptr = strtok(NULL, size_delim);
	if(ptr ==NULL){return 0;}
	...
	if(!strcmp(buff, "rgb")){
		set->type = RGB;
	} else if(!strcmp(buff, "b&w")){
		set->type = BW;
	} else if(!strcmp(buff, "b&w_alt")){
		set->type = BW_ALT;
	} else if(!strcmp(buff, "grey_stepped")){
		set->type = GREY_ST;
	} else if(!strcmp(buff, "grey_smoothed")){
		set->type = GREY_SM;
	} else{
		return 0;
	}
...
void image_save_rgb(struct render *set){
	...
	fout = fopen(strcat(set->basename, ".ppm"), "w");
	if ( fout == NULL ) {
		exit(1);
	}
