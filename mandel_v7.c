...
void image_save_grey(struct render *set){
	...
	for(i=0; i<set->height; i++){
		for(j=0; j<set->width; j++){			
			if(set->img[m2v(set, j, i)] == set->max_iter){
				fputc('0', fout);
			}else{
				fprintf(fout, "%d", set->img[m2v(set, j, i)]);
			}
...
void image_save_altern(struct render *set){
	...
	for(i=0; i<set->height; i++){
		for(j=0; j<set->width; j++){			
			if(set->img[m2v(set, j, i)] == set->max_iter || set->img[m2v(set, j, i)]%2 == 1){
				fputc('1', fout);
			}else{
				fputc('0', fout);
			}
